    ...
    public class Level
    {
        ...
        public void LoadMap(string filename, int levelId)
        {
            ...
            foreach(XmlNode soundNode in nodes)
            {
                ...
                new ResourceProxy().LoadSoundFromFile(name, file);
            }
            ...
            { 
                ...
                for (int y = 0; y < Columns; y++)
                {
                    ...
                    if (tile.Resource != "")
                        new ResourceProxy().LoadTextureFromFile(tile.Resource, @"resources\" + tile.Resource + ".png");


                    if(tile.Entity != "")
                    {
                        new ResourceProxy().LoadSpriteSheetFromFile(tile.Entity, @"resources\" + tile.Resource + ".png", tile.Frames);
                    }
